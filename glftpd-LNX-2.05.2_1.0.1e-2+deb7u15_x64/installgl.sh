...
if [ ! -f /etc/inetd.conf -a ! -f /etc/xinetd.conf -a "$os" != "darwin" ]; then
  echo "You do not have xinetd or inetd installed on your system. Please install"
  echo "and start one of them, then re-run this installer."
  exit 1
fi
...
[ ! "$usetcpd" = "y" ] && dummyflag="glftpd"

if [ -f /etc/inetd.conf -a -f /etc/xinetd.conf -a "$os" != "darwin" ]; then
    unset whichnetd
    until [ -n "$whichnetd" ]; do
        echo -n "Do you want to use [I]netd or [X]inetd? "
        read whichnetd
        case $whichnetd in
            [xX]) netd="x" ;;
            [iI]) netd="i" ;;
            *) unset whichnetd; continue ;;
        esac
    done
elif [ "$os" == "darwin" ]; then
    netd="l"
...
proc_createcert() {
  ...
  if [ $type = "dsa" ]; then
    ...
    $opensslbin rand -out gunk 1024 2>&1
    $opensslbin dsaparam -rand gunk -out $base.dsaparam 1024 2>&1
    $opensslbin dhparam -rand gunk -out $base.dh 1024 2>&1
